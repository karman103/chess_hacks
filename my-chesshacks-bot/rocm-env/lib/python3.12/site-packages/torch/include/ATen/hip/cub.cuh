...
inline void inclusive_scan(InputIteratorT input, OutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  ...
  for (int64_t i = max_cub_size; i < num_items; i += max_cub_size) {
   ...
   hipLaunchKernelGGL(( impl::transform_vals), dim3(1), dim3(1), 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA(), 
        output + i - 1,
        input + i,
        first_elem_ptr,
        scan_op);
    C10_HIP_KERNEL_LAUNCH_CHECK();
#if !CUB_SUPPORTS_FUTURE_VALUE()
    using ArgIndexInputIterator = NO_ROCM(at_cuda_detail)::hipcub::ArgIndexInputIterator<InputIteratorT>;
    ...
    auto input_ = NO_ROCM(at_cuda_detail)::hipcub::TransformInputIterator<input_t, decltype(input_iter_transform), ArgIndexInputIterator>(
...
inline void inclusive_sum_by_key(KeysInputIteratorT keys, ValuesInputIteratorT input, ValuesOutputIteratorT output, int64_t num_items) {
  ...
  CUB_WRAPPER(at_cuda_detail::hipcub::DeviceScan::InclusiveSumByKey,
      keys, input, output, num_items, at_cuda_detail::cub::Equality(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
}
...
inline void inclusive_scan_by_key(KeysInputIteratorT keys, ValuesInputIteratorT input, ValuesOutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  ...
  CUB_WRAPPER(at_cuda_detail::hipcub::DeviceScan::InclusiveScanByKey,
      keys, input, output, scan_op, num_items, at_cuda_detail::cub::Equality(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
}
