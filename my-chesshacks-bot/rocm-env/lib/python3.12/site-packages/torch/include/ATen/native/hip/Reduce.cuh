...
static void launch_reduce_kernel(const ReduceConfig& config, const R& reduction) {
  ...
  case 4:
   hipLaunchKernelGGL(( reduce_kernel<max_threads / 4, 4, R>), dim3(grid), dim3(block), shared_memory, stream, reduction);
    C10_HIP_KERNEL_LAUNCH_CHECK();
    break;
  case 2:
   hipLaunchKernelGGL(( reduce_kernel<max_threads / 2, 2, R>), dim3(grid), dim3(block), shared_memory, stream, reduction);
    C10_HIP_KERNEL_LAUNCH_CHECK();
    break;
  default:
   hipLaunchKernelGGL(( reduce_kernel<max_threads / 1, 1, R>), dim3(grid), dim3(block), shared_memory, stream, reduction);
    C10_HIP_KERNEL_LAUNCH_CHECK();
  }
}
