...
void im2col(
 ...
 hipLaunchKernelGGL(( im2col_kernel), dim3(GET_BLOCKS(num_kernels)), dim3(1024), 0, stream, 
      num_kernels,
      data_im,
      height,
      width,
...
void col2im(
  ...
  // CUDA_NUM_THREADS = 1024
 hipLaunchKernelGGL(( col2im_kernel<dt, accT>)
      , dim3(GET_BLOCKS(num_kernels, 512)), dim3(512), 0, stream, 
          num_kernels,
          data_col,
          height,
          width,
...
void col2im_batched(
  ...
  // bottom dimension, and then in the kernel add up the top dimensions.
  // CUDA_NUM_THREADS = 1024
 hipLaunchKernelGGL(( col2im_batched_kernel), dim3(GET_BLOCKS(output_numel, 512)), dim3(512), 0, stream, 
          num_kernels,
          data_col,
          col_batch_stride,
          nbatch,
